...
export async function saveChannel({ channelType, channelName, pageId, accessToken, webhookSecret }) {
  ...
  if (existing) {
    const updates = ['channel_name = ?', 'page_id = ?', 'is_active = 1'];
    const params  = [channelName, pageId];
    if (accessToken) { updates.push('access_token_encrypted = ?'); params.push(accessToken); }
    if (webhookSecret) { updates.push('webhook_secret_encrypted = ?'); params.push(webhookSecret); }
  ...
  const [result] = await pool.query(`
    INSERT INTO crm_channels (channel_type, channel_name, page_id, access_token_encrypted, webhook_secret_encrypted, is_active)
    VALUES (?, ?, ?, ?, ?, 1)
